...
import {
  ChevronLeft, ChevronRight, FileText, Folder, FolderOpen, Plus, LogOut,
  ChevronDown, ChevronRightIcon, Search,
} from 'lucide-react';

interface PageNode {
  id: string; title: string; is_folder: boolean;
  parent_id?: string | null; icon?: string | null;
  sort_order: number; children?: PageNode[];
}
...
function PageItem({ page, depth = 0, onDragStart, onDrop }: any) {
  ...
  return (
    <div draggable onDragStart={() => onDragStart?.(page.id)}
      onDragOver={(e: any) => { e.preventDefault(); }}
      onDrop={(e: any) => { e.preventDefault(); onDrop?.(page.id); }}>
      <button
        onClick={() => {
        ...
        className={`flex w-full items-center gap-1.5 rounded-md px-2 py-1 text-sm transition ${
          isSelected ? 'bg-neutral-100 text-neutral-900 dark:bg-neutral-800 dark:text-neutral-100'
      ...
      >
        ...
        {page.is_folder ? (
          expanded ? <FolderOpen className="h-3.5 w-3.5 text-neutral-400" />
      ...
      {expanded && hasChildren && (
        <div>
          {page.children!.map((child: PageNode) => (
            <PageItem key={child.id} page={child} depth={depth + 1} onDragStart={onDragStart} onDrop={onDrop} />
          ))}
        </div>
      )}
    </div>
...
export function Sidebar() {
  ...
  const handleNewPage = async () => {
    if (!activeWorkspaceId) return;
    const res = await api.pages.create({ workspace_id: activeWorkspaceId, title: 'Untitled', parent_id: null, is_folder: false });
  ...
  const handleDrop = useCallback(async (targetId: string) => {
    ...
    setPages(pages.map((p) => p.id === dragId ? { ...p, parent_id: newParentId } : p));
    setDragId(null);
  ...
  return (
    <>
      <aside className={`flex flex-col border-r border-neutral-100 bg-white transition-all duration-200 dark:border-neutral-800 dark:bg-neutral-950 ${
        ...
        <div className="flex flex-1 flex-col gap-0.5 overflow-auto px-1 py-1">
          {tree.map((page) => (
            <PageItem key={page.id} page={page} onDragStart={setDragId} onDrop={handleDrop} />
          ))}
        </div>
