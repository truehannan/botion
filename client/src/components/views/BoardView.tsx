...
export function BoardView() {
  ...
  const statusProp = properties.find((p) => p.name.toLowerCase().includes('status') && p.type === 'select') ?? properties.find((p) => p.type === 'select');
  const groups = statusProp ? (statusProp.config?.options ?? ['No Status']) : ['All'];
  const getGroupForRow = (row: any) => statusProp ? (row.properties[statusProp.id] ?? 'No Status') : 'All';
  ...
  return (
    <div className="flex h-full gap-3 overflow-auto p-3">
      {groups.map((group) => {
        ...
        return (
          <div key={group} className="flex w-72 shrink-0 flex-col rounded-lg bg-neutral-50 dark:bg-neutral-800/30"
            ...
            <div className="flex-1 space-y-1.5 p-2 pt-0">
              {groupRows.map((row) => (
                <div key={row.id} draggable onDragStart={() => setDragRowId(row.id)}
            ...
            <button className="flex items-center gap-1.5 px-3 py-2 text-xs text-neutral-400 hover:text-neutral-600"><Plus className="h-3.5 w-3.5" /> New</button>
          </div>
        );
      })}
    </div>
