...
export const CalloutBlock = createReactBlockSpec(
  {
    type: 'callout',
    propSchema: {
      type: { default: 'info' as keyof typeof calloutTypes },
  ...
  },
  {
    render: ({ block, contentRef }) => {
      const meta = calloutTypes[block.props.type as keyof typeof calloutTypes] ?? calloutTypes.info;
      const Icon = meta.icon;
      return (
        <div className={`flex items-start gap-3 rounded-lg px-4 py-3 ${meta.bg}`} data-callout-type={block.props.type}>
          <div className="mt-0.5 shrink-0 text-neutral-500 dark:text-neutral-400"><Icon className="h-4 w-4" strokeWidth={1.5} /></div>
