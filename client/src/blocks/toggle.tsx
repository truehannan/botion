...
export const ToggleBlock = createReactBlockSpec(
  {
    type: 'toggle',
    propSchema: { open: { default: true }, textAlignment: { default: 'left' }, backgroundColor: { default: 'default' }, textColor: { default: 'default' } },
    content: 'inline',
    hasChildren: true,
  },
  {
    render: ({ block, contentRef, editor }) => {
      ...
      return (
        <div data-block-type="toggle" className="my-1">
          <button onClick={() => { setOpen((o) => !o); editor.updateBlock(block, { props: { ...block.props, open: !open } }); }}
            ...
            <ChevronRight className={`h-3.5 w-3.5 shrink-0 text-neutral-400 transition-transform ${open ? 'rotate-90' : ''}`} />
            <span ref={contentRef} />
          </button>
          {open && block.children.length > 0 && (
            <div className="ml-5 border-l border-neutral-100 pl-3 dark:border-neutral-800">
              {block.children.map((child: any) => <div key={child.id} />)}
            </div>
          )}
        </div>
      );
